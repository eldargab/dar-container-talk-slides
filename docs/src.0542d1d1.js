...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"F51w":[function(require,module,exports) {
var define;
...
(function (root, factory) {
  ...
  function load() {
    var scripts = [],
        scriptsAsync = [],
        scriptsToPreload = 0; // Called once synchronous scripts finish loading
    ...
    for (var i = 0, len = config.dependencies.length; i < len; i++) {
      var s = config.dependencies[i]; // Load if there's no condition or the condition is truthy
    ...
    if (scripts.length) {
      scriptsToPreload = scripts.length; // Load synchronous scripts
  ...
  function createBackgrounds() {
    ...
    if (config.parallaxBackgroundImage) {
      ...
      dom.background.style.backgroundSize = config.parallaxBackgroundSize; // Make sure the below properties are set on the element - these properties are
  ...
  function createBackground(slide, container) {
    ...
    var element = document.createElement('div'); // Carry over custom classes from the slide to the background
    ...
    container.appendChild(element); // If backgrounds are being recreated, clear old classes

    slide.classList.remove('has-dark-background');
    slide.classList.remove('has-light-background');
    slide.slideBackgroundElement = element; // If this slide has a background color, add a class that
    ...
    if (computedBackgroundStyle && computedBackgroundStyle.backgroundColor) {
      var rgb = colorToRgb(computedBackgroundStyle.backgroundColor); // Ignore fully transparent backgrounds. Some browsers return
  ...
  function addEventListeners() {
    ...
    if (config.touch) {
      ...
      dom.wrapper.addEventListener('touchend', onTouchEnd, false); // Support pointer-style touch interaction as well
  ...
  function removeEventListeners() {
    ...
    dom.wrapper.removeEventListener('touchend', onTouchEnd, false); // IE11
  ...
  function layout() {
    if (dom.wrapper && !isPrintingPDF()) {
      var size = getComputedSlideSize(); // Layout the contents of the slides
  ...
  function slide(h, v, f, o) {
    ...
    updateParallax();
    updateSlideNumber();
    updateNotes(); // Update the URL hash
    ...
    if (slidesLength) {
      ...
      for (var i = 0; i < slidesLength; i++) {
        ...
        if (i < index) {
          ...
          if (config.fragments) {
            var pastFragments = toArray(element.querySelectorAll('.fragment')); // Show all fragments on prior slides
          ...
          if (config.fragments) {
            var futureFragments = toArray(element.querySelectorAll('.fragment.visible')); // No fragments in future slides should be visible ahead of time
  ...
  function updateSlideNumber() {
    ...
    if (config.slideNumber && dom.slideNumber) {
      var value = [];
      var format = 'h.v'; // Check if a custom number format is available
  ...
  function updateBackground(includeAll) {
    ...
    if (currentBackground) {
      ...
      var backgroundImageURL = currentBackground.style.backgroundImage || ''; // Restart GIFs (doesn't work in Firefox)
  ...
  function loadSlide(slide, options) {
    ...
    if (background) {
      background.style.display = 'block'; // If the background contains media, load it
  ...
  function formatEmbeddedContent() {
    ...
    _appendParamToIframeSource('data-src', 'player.vimeo.com/', 'api=1'); // Always show media controls on mobile devices
  ...
  function readURL() {
    ...
    var bits = hash.slice(2).split('/'),
        name = hash.replace(/#|\//gi, ''); // If the first bit is invalid and there is a name we can
      ...
      if (fragments.length) {
        ...
        toArray(fragments).forEach(function (element, i) {
          ...
          if (i <= index) {
            ...
            element.classList.add('visible');
            element.classList.remove('current-fragment'); // Announce the fragments one by one to the Screen Reader
  ...
  function onDocumentKeyDown(event) {
    ...
    onUserInput(event); // Check if there's a focused element that could be using
    // the keyboard

    var activeElementIsCE = document.activeElement && document.activeElement.contentEditable !== 'inherit';
    var activeElementIsInput = document.activeElement && document.activeElement.tagName && /input|textarea/i.test(document.activeElement.tagName);
    var activeElementIsNotes = document.activeElement && document.activeElement.className && /speaker-notes/i.test(document.activeElement.className); // Disregard the event if there's a focused element or a
  ...
  function onTouchStart(event) {
    ...
    touch.startCount = event.touches.length; // If there's two touches we need to memorize the distance
  ...
  function onTouchMove(event) {
    ...
    if (!touch.captured) {
      ...
      var currentY = event.touches[0].clientY; // If the touch started with two points and still has
...
var _reveal = _interopRequireDefault(require("../reveal/reveal.js"));
...
//# sourceMappingURL=src.0542d1d1.map